...
export default function App() {
  ...
  const [brokerStatuses, setBrokerStatuses] = useState<Record<string, BrokerStatus>>({
    ...
    broker3: { name: 'Mosquitto (Authenticated)', url: 'wss://test.mosquitto.org:8092/mqtt', connected: false, ping: null, error: null },
  });
  ...
  const connectBrokers = useCallback(() => {
    ...
    try {
      ...
      client1.on('message', (topic: string, messageBuffer: any) => {
        ...
        // 3. Deteksi sesi ganda
          ...
          if (status === 'online' && cid && cid !== myClientId1Ref.current) {
            ...
            if (client3Ref.current) {
              ...
              setBrokerStatuses(prev => ({
                ...prev,
                broker3: { ...prev.broker3, connected: false }
              }));
            }
          }
        }
    ...
    const broker3Url = 'wss://test.mosquitto.org:8092/mqtt';
    try {
      ...
      client3.on('connect', () => {
        setBrokerStatuses(prev => ({
          ...prev,
          broker3: { ...prev.broker3, connected: true, error: null }
        }));
        addLog('Broker 3 (Mosquitto Auth) terhubung', 'incoming');
      ...
      client3.on('message', (topic: string, messageBuffer: any) => {
        ...
        if (topic === `iot/ping/3/${myClientId1Ref.current}`) {
          try {
            ...
            setBrokerStatuses(prev => ({
              ...prev,
              broker3: { ...prev.broker3, ping: latency }
            }));
          } catch (e) {}
        }
      });
      ...
      client3.on('error', (err: any) => {
        setBrokerStatuses(prev => ({
          ...prev,
          broker3: { ...prev.broker3, error: err.message, connected: false }
        }));
        addLog(`Broker 3 Error: ${err.message}`, 'error');
      });
  ...
  useEffect(() => {
    const intv = setInterval(() => {
      ...
      publishPing(3, client3Ref.current);
    }, 6000);
  ...
  const handleVoiceCommand = (cmd: string) => {
    ...
    if (allRelaysOnPhrases.some(phrase => cmd.includes(phrase))) {
      setAllRelays(true);
      return;
    }
    ...
    if (allRelaysOffPhrases.some(phrase => cmd.includes(phrase))) {
      setAllRelays(false);
      return;
    }

    // 3. INDIVIDUAL RELAYS ON COMMANDS
    const relay1On = ['relay satu nyala', 'hidupkan relay satu', 'relay satu on', 'relay 1 nyala', 'relay 1 on', 'aktifkan relay satu', 'nyalakan relay satu', 'relay pertama nyala'];
    const relay2On = ['relay dua nyala', 'hidupkan relay dua', 'relay dua on', 'relay 2 nyala', 'relay 2 on', 'aktifkan relay dua', 'nyalakan relay dua', 'relay kedua nyala'];
    const relay3On = ['relay tiga nyala', 'hidupkan relay tiga', 'relay tiga on', 'relay 3 nyala', 'relay 3 on', 'aktifkan relay tiga', 'nyalakan relay tiga', 'relay ketiga nyala'];
    const relay4On = ['relay empat nyala', 'hidupkan relay empat', 'relay empat on', 'relay 4 nyala', 'relay 4 on', 'aktifkan relay empat', 'nyalakan relay empat', 'relay keempat nyala'];

    if (relay1On.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 1 ? { ...r, active: true } : r);
      publishToAllBrokers('iot/relay/1', 'ON');
      saveRelays(updated);
      speakTts('Relay satu dinyalakan');
      return;
    }
    if (relay2On.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 2 ? { ...r, active: true } : r);
      publishToAllBrokers('iot/relay/2', 'ON');
      saveRelays(updated);
      speakTts('Relay dua dinyalakan');
      return;
    }
    if (relay3On.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 3 ? { ...r, active: true } : r);
      publishToAllBrokers('iot/relay/3', 'ON');
      saveRelays(updated);
      speakTts('Relay tiga dinyalakan');
      return;
    }
    if (relay4On.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 4 ? { ...r, active: true } : r);
    ...
    const relay1Off = ['relay satu mati', 'matikan relay satu', 'relay satu off', 'relay 1 mati', 'relay 1 off', 'nonaktifkan relay satu', 'padamkan relay satu', 'relay pertama mati'];
    const relay2Off = ['relay dua mati', 'matikan relay dua', 'relay dua off', 'relay 2 mati', 'relay 2 off', 'nonaktifkan relay dua', 'padamkan relay dua', 'relay kedua mati'];
    const relay3Off = ['relay tiga mati', 'matikan relay tiga', 'relay tiga off', 'relay 3 mati', 'relay 3 off', 'nonaktifkan relay tiga', 'padamkan relay tiga', 'relay ketiga mati'];
    const relay4Off = ['relay empat mati', 'matikan relay empat', 'relay empat off', 'relay 4 mati', 'relay 4 off', 'nonaktifkan relay empat', 'padamkan relay empat', 'relay keempat mati'];

    if (relay1Off.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 1 ? { ...r, active: false } : r);
      publishToAllBrokers('iot/relay/1', 'OFF');
      saveRelays(updated);
      speakTts('Relay satu dimatikan');
      return;
    }
    if (relay2Off.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 2 ? { ...r, active: false } : r);
      publishToAllBrokers('iot/relay/2', 'OFF');
      saveRelays(updated);
      speakTts('Relay dua dimatikan');
      return;
    }
    if (relay3Off.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 3 ? { ...r, active: false } : r);
      publishToAllBrokers('iot/relay/3', 'OFF');
      saveRelays(updated);
      speakTts('Relay tiga dimatikan');
      return;
    }
    if (relay4Off.some(p => cmd.includes(p))) {
      const updated = relayConfigs.map(r => r.id === 4 ? { ...r, active: false } : r);
    ...
    if (allPolasOn.some(phrase => cmd.includes(phrase))) {
      setAllPatterns(true);
      return;
    }
    ...
    if (allPolasOff.some(phrase => cmd.includes(phrase))) {
      setAllPatterns(false);
      return;
    }

    // 7. POLA 1 ON / OFF
    const pola1On = ['hidupkan pola satu', 'pola satu nyala', 'aktifkan pola satu', 'pola 1 nyala', 'nyalakan pola satu', 'pola satu on', 'pola 1 on', 'jalankan pola satu'];
    const pola1Off = ['matikan pola satu', 'stop pola satu', 'pola satu mati', 'pola 1 mati', 'pola 1 off', 'nonaktifkan pola satu', 'hentikan pola satu'];

    if (pola1On.some(p => cmd.includes(p))) {
      const updated = polaConfigs.map(p => p.id === 1 ? { ...p, active: true } : p);
      setPolaConfigs(updated);
      publishToAllBrokers('iot/pola/1', 'ON');
      saveRelays(relayConfigs.map(r => ({ ...r, active: false })));
      speakTts('Pola satu dinyalakan, pola kiri ke kanan aktif');
      return;
    }
    if (pola1Off.some(p => cmd.includes(p))) {
      const updated = polaConfigs.map(p => p.id === 1 ? { ...p, active: false } : p);
      setPolaConfigs(updated);
    ...
    // 8. POLA 2 ON / OFF
    const pola2On = ['hidupkan pola dua', 'pola dua nyala', 'aktifkan pola dua', 'pola 2 nyala', 'nyalakan pola dua', 'pola dua on', 'pola 2 on', 'jalankan pola dua'];
    const pola2Off = ['matikan pola dua', 'stop pola dua', 'pola dua mati', 'pola 2 mati', 'pola 2 off', 'nonaktifkan pola dua', 'hentikan pola dua'];

    if (pola2On.some(p => cmd.includes(p))) {
      const updated = polaConfigs.map(p => p.id === 2 ? { ...p, active: true } : p);
      setPolaConfigs(updated);
    ...
    if (pola2Off.some(p => cmd.includes(p))) {
      const updated = polaConfigs.map(p => p.id === 2 ? { ...p, active: false } : p);
      setPolaConfigs(updated);
    ...
    if (queryTemp.some(p => cmd.includes(p))) {
    ...
    if (queryHumid.some(p => cmd.includes(p))) {
    ...
    if (querySensorAll.some(p => cmd.includes(p))) {
      const activeSuhu = suhuRef.current;
      const activeKelembapan = kelembapanRef.current;
    ...
    if (cmd.includes('bersihkan log') || cmd.includes('hapus log') || cmd.includes('clear log')) {
      setLogs([]);
      speakTts('Log dibersihkan');
      return;
    }

    if (cmd.includes('semua mati') || cmd.includes('matikan semua') || cmd.includes('shutdown')) {
      handleShutdownAll();
      return;
    }
